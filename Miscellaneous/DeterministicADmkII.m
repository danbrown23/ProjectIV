...
X(1:n0) = effectivecarryingcapacity(x0,X0,sd_K,sd_a,n0,tspan); 
survivors = find(X~=0);
xeff = x(:,survivors); 
Xeff = X(survivors);
...
for i = 2:steps
    ...
    X(survivors) = effectivecarryingcapacity(xeff,Xeff,sd_K,sd_a,n,tspan); % ecological extinction
    survivors = find(X~=0);
    n = length(survivors);
    ...
    if i - count > branchcd-1 % branch event
        ...
        X([survivors nbound+survivors]) = effectivecarryingcapacity(x(:,[survivors nbound+survivors]),...
                                                             ...
                                                             length([survivors nbound+survivors]),tspan); % calculate ecological equilibrium
        nbound = 2*nbound;
        count = i;
    end
    survivors = find(X~=0);
    ...
    f = fitness(xeff,y,Xeff,sd_K,sd_a,n);
    mutant = find(f>0);
