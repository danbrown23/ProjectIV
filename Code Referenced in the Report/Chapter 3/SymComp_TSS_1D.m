...
% x0 = 2*rand(1,500)-ones(1,500);
x0 = 0;
X0 = 0.01.*ones(n0,1); % initial population vector
...
% given a phenotype matrix and an initial population vector, returns the effective carrying capacity vector 
    r = GrowthRate(x);
    K = CarryingCapacity(x,var_K,var_a);
...
function f = Fitness(x,y,X,var_K,var_a)
    ...
    f = GrowthRate(y).*(1-beta*X./CarryingCapacity(y,var_K,var_a));
end
