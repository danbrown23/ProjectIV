...
import Functions.EffectiveCarryingCapacity
...
% variances
sd_a = 0.25; % competition variance
sd_K = 1; % carrying capacity variance
...
mergedist = 0.02; % the threshhold average distance between two trajectories for merging
mergetime = 20; % steps before they are merged for computational efficiency
...
X(1:n0) = EffectiveCarryingCapacity(x0,X0,sd_K,sd_a,tspan,1); 
survivors = find(X~=0);
xeff = x(:,survivors); 
Xeff = X(survivors);
...
for i = 2:steps
    ...
    X(survivors) = EffectiveCarryingCapacity(xeff,Xeff,sd_K,sd_a,tspan,i); % ecological extinction
    survivors = find(X~=0);
    n = length(survivors);
    ...
    %     end
    % end
    if i - count > branchcd-1 & n < nmax/2 % branch event
        disp("branched!");
        ...
        X([survivors nbound+survivors]) = EffectiveCarryingCapacity(x(:,[survivors nbound+survivors]),...
                                                             ...
                                                             tspan,i); % calculate ecological equilibrium
        nbound = 2*nbound;
        count = i;
    end
    survivors = find(X~=0);
    ...
    f = Fitness(xeff,y,Xeff,sd_K,sd_a,i);
    mutant = find(f>0);
