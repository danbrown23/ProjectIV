...
function K = CarryingCapacity(x,var_K,v,t,tstart)
...
function X = EffectiveCarryingCapacity(x,X0,var_K,var_a,tspan,v,t,tstart) % (allowing extinction)
% given a phenotype matrix and an initial population vector, returns the effective carrying capacity vector 
    r = GrowthRate(x);
    K = CarryingCapacity(x,var_K,v,t,tstart);
...
function f = Fitness(x,y,X,var_K,var_a,v,t,tstart)
    ...
    f = GrowthRate(y).*(1-beta*X./CarryingCapacity(y,var_K,v,t,tstart));
end
...
X(1:n0) = EffectiveCarryingCapacity(x0,X0,var_K,var_a,tspan,v,1,tstart);
survivors = find(X~=0);
...
while i < steps && n < divlim && n > 0
    ...
    f = Fitness(xeff,y,Xeff,var_K,var_a,v,i,tstart);
    ...
    X(survivors) = EffectiveCarryingCapacity(xeff,Xeff,var_K,var_a,tspan,v,i,tstart); % ecological extinction
    survivors = find(X~=0);
    % adjust data
    ...
    % size(xpts(:,i))
    % size(xeff)
    % flist(:,i) = Fitness(xeff,xpts(:,i)',Xeff,var_K,var_a,v,i,tstart);
    if i > tstart
    CCClist(i) = v.*i;
    end
end
