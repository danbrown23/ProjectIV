...
function X = EffectiveCarryingCapacity(x,X0,var_K,var_a,tspan,b,t,tstart) % (allowing extinction)
% given a phenotype matrix and an initial population vector, returns the effective carrying capacity vector 
    r = GrowthRate(x);
    K = CarryingCapacity(x,var_K,b,t,tstart);
...
function f = Fitness(x,y,X,var_K,var_a,b,t,tstart)
    ...
    f = GrowthRate(y).*(1-beta*X./CarryingCapacity(y,var_K,b,t,tstart));
end
...
X(1:n0) = EffectiveCarryingCapacity(x0,X0,var_K,var_a,tspan,b,1,tstart);
survivors = find(X~=0);
...
while i < steps && n < divlim && n > 0
    ...
    f = Fitness(xeff,y,Xeff,var_K,var_a,b,i,tstart);
    ...
    X(survivors) = EffectiveCarryingCapacity(xeff,Xeff,var_K,var_a,tspan,b,i,tstart); % ecological extinction
    survivors = find(X~=0);
    % adjust data
    ...
    % size(xpts(:,i))
    % size(xeff)
    % flist(:,i) = Fitness(xeff,xpts(:,i)',Xeff,var_K,var_a,b,i,tstart);
end
toc
...
% x = trajectory(:,:,frame);
% CCC = CCClist(frame);
% h = histogram(x,-w+CCC:0.05:w+CCC);
% e = h.BinEdges;
...
% fframes = 30;
% for k = 1:frames
%     steprange = fend-fstart;
%     frame = k*floor(steprange/frames)+fstart;
%     plot(xpts,f(:,frame))
% end
