...
function beta = velocity(A,omega,t,tstart)
if t > tstart
    t = t-tstart;
    beta = A*sin(omega*t);
else
...
function r = GrowthRate(x,y,c,R_0,w,s,T,A,omega,t,tstart)
r = R_0*w*s./(sqrt(s^2+y.^2)).*exp(-(c.*y)-(x-velocity(A,omega,t,tstart)).^2./(2.*(s.^2+y.^2)))-T;
r = r(:);
end

function K = CarryingCapacity(x,y,c,R_0,w,s,T,r_R,A,omega,t,tstart)
K = r_R.*y.*sqrt(2.*pi*(2.*s^2+y.^2)).*exp(c.*y).*exp((x-velocity(A,omega,t,tstart)).^2./(2*s^2+y.^2)).*(exp(-(x-velocity(A,omega,t,tstart)).^2./(2*(s^2+y.^2)))./(sqrt(s^2+y.^2))-(T.*exp(c.*y)/(R_0.*w.*s)));
K = K(:);
end

function alpha = Competition(x,y,c,s,A,omega,t,tstart) % competition i experiences from j
n = length(x);
alpha = zeros(n,n);
for i =1:n
    for j = 1:n
        alpha(i,j) = y(i).*sqrt((2.*s.^2+y(i).^2)./(y(i).^2.*y(j).^2+s^2.*(y(i).^2+y(j).^2))).*exp(c.*(y(i)-y(j))+(x(i)-velocity(A,omega,t,tstart)).^2./(2.*s^2+y(i).^2)-((x(j)-velocity(A,omega,t,tstart)).^2.*y(i).^2+(x(i)-velocity(A,omega,t,tstart)).^2.*y(j).^2+s^2.*(x(j)-x(i)).^2)./(2.*(y(j).^2.*y(i).^2+s^2.*(y(j).^2+y(i).^2))));
    end
end
end
...
steps = 24000; % evolutionary timespan
...
function X = EffectiveCarryingCapacity(x,y,c,R_0,w,s,T,r_R,X0,tspan,A,omega,t,tstart) % (allowing 
% extinction) given a phenotype matrix and an initial population vector, returns the 
% effective carrying capacity vector 
    r = GrowthRate(x,y,c,R_0,w,s,T,A,omega,t,tstart);
    K = CarryingCapacity(x,y,c,R_0,w,s,T,r_R,A,omega,t,tstart);
    alpha = Competition(x,y,c,s,A,omega,t,tstart);
...
% fitness
function f = Fitness(u,v,x,y,X,c,R_0,w,s,T,r_R,A,omega,t,tstart)
n = length(x);
ux = [x,u];
vy = [y,v];
X = X(:);
r = GrowthRate(u,v,c,R_0,w,s,T,A,omega,t,tstart);
K = CarryingCapacity(u,v,c,R_0,w,s,T,r_R,A,omega,t,tstart);
alpha = Competition(ux,vy,c,s,A,omega,t,tstart);
...
X(1:n0) = EffectiveCarryingCapacity(x0(1,:),x0(2,:),c,R_0,w,s,T,r_R,X0,tspan,A,omega,1,tstart);
...
for i = 2:steps
    ...
    f = Fitness(fueff,fveff,fxeff,fyeff,Xeff,c,R_0,w,s,T,r_R,A,omega,i,tstart);
    ...
    X(survivors) = EffectiveCarryingCapacity(fxeff,fyeff,c,R_0,w,s,T,r_R,Xeff,tspan,A,omega,i,tstart);
    survivors = find(X~=0);
    % update data
...
figure
plot(1:steps,nlist(1:steps))
xlabel("Time Steps")
ylabel("Number of Distinct Species")

% trajectory against time
figure
CCC(1:tstart) = 0;
CCC(tstart:steps) = A*sin(omega*((tstart:steps)-tstart));
plot(1:steps,CCC)
...
figure
popsums = sum(PopulationData(:,1:steps),1);
plot(1:steps,popsums);
xlabel("Time Steps")
ylabel("Total Population")

% population distribution frames
framesize = floor((fend-fstart)/frames);

for i = 1:frames
    frame = framesize*(i-1) + fstart;
    figure
    xdata = trajectory(:,:,frame);
    Xdata = PopulationData(:,frame);
    hold on
    for j = 1:size(xdata,2)
        if Xdata(j)>0
            scatter(xdata(1,j),xdata(2,j),1000*Xdata(j),'filled')
        end
    end
    CCC = velocity(A,omega,frame,tstart);
    plot(CCC,0,'-o',LineWidth=4,color = 'blue')
    hold off
    xlim([-width,width])
    % ylim([0,l])
    title("Time =", frame)
