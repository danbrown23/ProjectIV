...
function r = GrowthRate1(x)
    r = 1;
end
function K = CarryingCapacity1(x,var_K)
    K = exp(-abs(x).^2)./(2*var_K);
end
function alpha = Competition1(x,y,var_a)
    alpha = exp(-abs(x-y).^2./(2*var_a));
end
function f = Fitness1(x,y,var_a,var_K)
f = GrowthRate1(x).*(1-Competition1(x,y,var_a).*CarryingCapacity1(x,var_K)./CarryingCapacity1(y,var_K));
end


[x,y] = meshgrid(-width:res:width,-3:res:length);
for i = 1:frames
    ...
    y(1) = trajectory(1,1,frame);
    y(2) = trajectory(2,1,frame);

    figure
    f = Fitness1(y,x,var_a,var_K);
    surf(x,y,f,FaceAlpha = faceop, EdgeColor = 'interp',EdgeAlpha = edgeop);
    hold on
    scatter3(y(1),y(2),0,'filled','LineWidth',5,MarkerFaceColor='red',MarkerEdgeColor='red')
