...
steps = 20000; % evolutionary timespan
...
popdata = zeros(steps,length(omegalist));
ndata = zeros(steps,length(omegalist));
mdata = NaN(2,steps,length(omegalist));
vardata = NaN(2,steps,length(omegalist));
tdata = zeros(steps,length(omegalist));
for bindex = 1:length(omegalist)

    omega = omegalist(bindex);
    ...
    while i < steps && n > 0
        ...
        CCClist(i) = velocity(A,omega,i,tstart);
    end
    toc
