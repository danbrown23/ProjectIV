classdef SeasonalSymmetricCompetition_Functions

    %% phenotype parametrisations

    methods(Static)

        function speed = v(t,d)
            % given a time t, returns the d dimensional vector of the centre of the carrying capacity
            speed = 0.001*ones(d,1).*t;
        end
        ...
        function K = carryingcapacity(x,i,sd_K,t)
            ...
            K = exp(-norm(x(:,i)-v(t,length(x(:,i)))).^2./(2.*sd_K^2));
        end
        ...
        function X = effectivecarryingcapacity(x,X0,sd_K,sd_a,n,tspan,t) % (allowing extinction)
        ...
        function f = fitness(x,y,X,sd_K,sd_a,n,t)
            ...
            beta = competition(phenotype,n+1:2*n,1:n,sd_a);
            % compute the fitness
